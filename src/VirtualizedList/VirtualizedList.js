...
const VirtualizedList = ({ wrapperTagName, wrapperStyle, itemHeight, items, renderItem }) => {
  ...
  useEffect(() => {
    ...
    // When this effect is run: Calculate which list-items should currently be visible
  ...
  const resetWrapperElHeight = () => {
    if (wrapperElRef.current) {
      setWrapperElHeight(wrapperElRef.current.offsetHeight);
    }
  };
